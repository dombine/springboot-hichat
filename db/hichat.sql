...
CREATE VIEW `permissionlist` AS select `t`.`id` AS `pId`,`t`.`menu_name` AS `pLabel`,`child`.`id` AS `cId`,`child`.`permission_name` AS `cLabel` from (`permissionmenu` `t` left join `h_permission` `child` on((`child`.`menu_code` = `t`.`menu_code`))) order by `child`.`id` ;
